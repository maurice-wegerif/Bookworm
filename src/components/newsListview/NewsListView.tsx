...
interface BookListNewsProps {
  books: Book[];
  heading: String;
}

export const BookListNews = ({ books, heading }: BookListNewsProps) => {
  return (
    <div className="mb-8">
        ...
        {books.map((book, index) => (
          ...
          >
            <div className={`flex flex-col justify-start ${index === 0 ? "pr-4" : ""}`}>
              ...
              <span className="font-serif font-bold text-text mb-2">{book.year?.toString()}</span>
              {index === 0 && (
